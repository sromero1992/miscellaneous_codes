C/ FIGURE 6.2.5
      PARAMETER (M=100,N=(M-1)**3,NZMAX=7*N)
...
C                     DISTRIBUTE L-TH COLUMN TO PROCESSOR MOD(L-1,NPES)
            ...
            ENDIF
         ENDIF
	 write(*,*) "The actual NZ is :", NZ
	 write(*,*) "The actual IROW(NZ) and ICOL(NZ) are : " ,IROW(NZ),JCOL(NZ)
	 write(*,*) "The sparese matrix AS(1:NZ) is :", AS(1:4)
         B(L) = H**2
         D(L) = 1.0
   10 CONTINUE
         allocate(A(NZ,NZ))
         ...
         do NI=1,NZ
              A(NI,NI)=AS(NI) !not sure
         end do
	 write(*,*) "The matrix A is: "
	 do kk=1,NZ
	 write(*,*) (A(kk,jj),jj=1,NZ)
!      CALL PCG(AS,IROW,JCOL,NZ,X,B,N,D)
!!      CALL PLINEQ(AS,JCOL,X,B)
