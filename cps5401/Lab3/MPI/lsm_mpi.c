#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
#include <math.h>
#include <string.h>
#include <mpi.h>
#define NB 20

// The basis function, j=0,...,NB
// j=0,...,NB/2,   basis is cos(jx) 
// j=NB/2+1,..,NB, basis is sin((j-NB/2)x)
double ls_basis(int j, double x);

//DGESV solves a real linear equation 
extern void dgesv_(int *N, int *NRHS, double *A, int *LDA, int *IPIV, double *B, int *LDB, int *INFO);

int main(int argc, char** argv) {

  int n; // Number of data pair (x_i, y_i);
  int ID_proc ,num_procs ,source, target; 
  double* x;
  double* y;
  int tag = 0;
  int root = 0;
  // Preparation
  assert( NB%2 == 0 );
  
  MPI_Init( &argc , &argv );//initialiaze mpi
  MPI_Comm_rank( MPI_COMM_WORLD, &ID_proc);
  MPI_Comm_size( MPI_COMM_WORLD, &num_procs);//total of processors to use
  MPI_Status status;
  // Part (1) -- Read the data

  // if ( ID_proc == root ){

  FILE* data_file = fopen("data.txt", "r");
  fscanf( data_file, "%d\n", &n );
  x = (double*) malloc( n * sizeof( double ) );
  y = (double*) malloc( n * sizeof( double ) );
  
  for ( int i = 0; i < n; i++ ) 
    fscanf( data_file, "%lf\t%lf\n", x+i, y+i );
  fclose( data_file );
 //}
 //MPI_Bcast(&n, 1, MPI_INT, root, MPI_COMM_WORLD );
 
  double m = n/num_procs;//distribution of the work
  int start = ID_proc*m;
  int end = ( ID_proc + 1 )*m;
  
  // Part (2) -k- Assemble the matrix
  double* A; // Size is (NB+1)*(NB+1);
  double* B; // Size is (NB+1)
  double* a; // Size is (NB+1)
  
  // Allocate and initialize to zero
  A = (double*) calloc( (NB+1) * (NB+1), sizeof( double ) );
  B = (double*) calloc( (NB+1), sizeof( double ) );
  a = (double*) calloc( (NB+1), sizeof( double ) );
  
  double* f; // Size is NB+1
  f = (double*) malloc( (NB+1) * sizeof( double ) );

  // Outer loop: iterate over each data pair
  if (ID_proc != root){
     if (num_procs == 2){
        for ( int i = 0; i < n; i++ ) {
        // All basis functions at x_i
        for ( int j = 0; j <= NB; j++ )
          f[j] = ls_basis(j,x[i]);
        // Assemble in B
        for ( int j = 0; j <= NB; j++ )
          B[j] += y[i] * f[j];
        // Assemble in A
        }
     }
     else {//case for more than 2 process
        source = 1; 
        target = num_procs;
      for( source = 1; num_procs-1 ; source++){
       for ( int i = start; i < end; i++ ) {
         
          // All basis functions at x_i
          if (ID_proc != source){
              for ( int j = 0; j <= NB; j++ ){
           	 f[j] = ls_basis(j,x[i]);
            	 MPI_Send( &f, m, MPI_DOUBLE, target, tag, MPI_COMM_WORLD );
           	 MPI_Recv( &f, m, MPI_DOUBLE, source, tag, MPI_COMM_WORLD, &status  );
              }
	  }
         // Assemble in B
         if (ID_proc == source){
              for ( int j = 0; j <= NB; j++ ){
          	 B[j] += y[i] * f[j];
	  	 MPI_Recv( &f, m, MPI_DOUBLE, source, tag, MPI_COMM_WORLD, &status  );
              }
         }
        }
       target--;
       }
     }
  }
  else{
    for ( int j1 = 0; j1 <= NB; j1++ )
      for ( int j2 = 0; j2 <= NB; j2++ ){
         MPI_Recv( &f, n, MPI_DOUBLE, source, tag, MPI_COMM_WORLD, &status  );
         A[j1*(NB+1)+j2] += f[j1] * f[j2];
      }     
  }
   

  // Part (3) -- Solve for the coefficients and report
  // Use Lapack to solve for Aa=B
  int  N    = NB;
  int  NRHS = 1;
  int  LDA  = NB;
  int* IPIV = (int*) malloc( NB * sizeof( int ) );
  int  LDB  = NB;
  int  INFO;
  double* MAT = (double*) malloc( NB * NB * sizeof( double ) );
  double* RHS = (double*) malloc( NB * sizeof( double ) );
  memcpy( MAT, A,  NB * NB * sizeof(double) );
  memcpy( RHS, B,  NB * sizeof(double) );
  dgesv_(&N, &NRHS, &(MAT[0]), &LDA, &(IPIV[0]), &(RHS[0]), &LDB, &INFO);

  // Print out coefficients in a
  if (ID_proc == root){
   printf("The fitted coefficients are:\n");
   for ( int j = 0; j <= NB; j++ ){
     a[j]=RHS[j];
     printf("\t%f,", a[j]);
   printf("\n");
   }
  }
  
  // Clean-up
  free( x );
  free( y );
  free( f );

  free( A );
  free( B );
  free( a );
  
  free( MAT );
  free( RHS );
  free( IPIV );

  MPI_Finalize();
  return 0;
}

double ls_basis(int j, double x) {
  assert( (j>=0) && (j<=NB) );
  if ( j<=NB/2 ) // cos
    return cos(j*x);
  else // sin
    return sin((j-0.5*NB)*x);
}

